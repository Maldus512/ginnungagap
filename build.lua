...
local CFLAGS = "-nostartfiles -nostdlib -ffreestanding -O0 -g -mthumb -Wall -Wextra -pedantic " ..
    FCPU .. " " .. INCLUDES
local LFLAGS = "-nostartfiles -mthumb " .. FCPU
...
---@return integer[]
local padTo = function(data, paddingSize)
    if #data <= paddingSize then
        while #data < paddingSize do
            table.insert(data, 0)
        end
    end
    return data
...
local crc32 = function(data)
    ...
    for _, byte in pairs(data) do
        local tableOffset = (crc >> 24) & 0xFF
        tableOffset = (tableOffset ~ byte) + 1
        crc = ((crc << 8) & 0xFFFFFFFF) ~ crcTable[tableOffset]
    end
...
---@param data integer[]
---@return integer[]
local generateUf2 = function(data)
    local PAGE_SIZE = 256 -- Page size supported by Pi Pico

    local addBlock = function(uf2, chunk, address, blockNumber, totalBlocks)
        local preamble = {
            0x0A324655,  -- Abra
            0x9E5D5157,  -- Kadabra
            0x00002000,  -- Flags
            ...
            totalBlocks, -- Blocks in file
            0XFF56E48B,  -- Family ID
        }

        for _, word in pairs(preamble) do
            table.insert(uf2, word & 0xFF)
            table.insert(uf2, (word >> 8) & 0xFF)
            table.insert(uf2, (word >> 16) & 0xFF)
            table.insert(uf2, (word >> 24) & 0xFF)
        end

        for _, byte in pairs(chunk) do
            table.insert(uf2, byte)
        end

        local alakazam = 0x0AB16F30

        table.insert(uf2, alakazam & 0xFF)
        table.insert(uf2, (alakazam >> 8) & 0xFF)
        table.insert(uf2, (alakazam >> 16) & 0xFF)
        table.insert(uf2, (alakazam >> 24) & 0xFF)
    end
    ...
    while blocks * PAGE_SIZE < totalSize do
        local chunk = table.move(data, blocks * PAGE_SIZE + 1, (blocks + 1) * PAGE_SIZE, 1, {})
        addBlock(uf2, chunk, 0x10000000 + blocks * PAGE_SIZE, blocks, math.ceil(#data / PAGE_SIZE))
        blocks = blocks + 1
    end
...
return {
    configure = {
        ...
        action = function()
            ...
            local buildConf = svadilfari.newBuildConfiguration { buildFolder = "build" }

            local gcc = buildConf:rule {
                name = "gcc",
            ...
            local compileC = function(path)
                return buildConf:step(gcc {
                    input = path,
                    target = svadilfari.toExtension("o")
                })
            end

            local ld = buildConf:rule {
                name = "link",
                command = LINKER .. " " .. LFLAGS .. " $linkerScript -Wl,-Map=$out.map $in -o $out $libraries",
            }
            local objcopy = buildConf:command(OBJCOPY .. " -O binary $in $out")

            -- Build Stage 2 bootloader
            local boot_stage2_objects = compileC(COMPONENT_BOOT_STAGE2 .. "/boot_stage2.c")

            local boot_stage2_targets = buildConf:pipe {
                ld {
                    input = boot_stage2_objects,
                    target = COMPONENT_BOOT_STAGE2 .. "/boot_stage2.elf",
                    dependencies = { BOOT_STAGE2_MEMMAP },
                    env = { linkerScript = "-T " .. BOOT_STAGE2_MEMMAP },
                },
                objcopy {
                    target = svadilfari.toExtension("bin"),
                },
                buildConf:command("svad.lua stage2 $in $out") {
                    target = function(input) return input:gsub("%.bin$", "_checked.s") end,
                },
                gcc {
                    target = svadilfari.toExtension("o"),
                }
            }
            local boot_stage2_checked_object = boot_stage2_targets[#boot_stage2_targets]
            ...
            for _, source in pairs(svadilfari.listFilesOfType("components", "c", { COMPONENT_BOOT_STAGE2 })) do
                table.insert(objects, compileC(source))
            end

            local finalOutputs = buildConf:pipe {
                ld {
                    input = objects,
                    target = "ginnungagap.elf",
                    alias = "elf",
                    dependencies = { MAIN_MEMMAP },
                    env = { linkerScript = " --specs=nosys.specs -T " .. MAIN_MEMMAP, libraries = "-lgcc" },
                },
                objcopy {
                    target = svadilfari.toExtension("bin"),
                    alias = "bin",
                },
                buildConf:command("svad.lua uf2 $in $out") {
                    target = svadilfari.toExtension("uf2"),
                    alias = "uf2",
                },
            }
            ...
            buildConf:run {
                name = "gdbrun",
                input = finalOutputs[1],
                command = GDB .. " -x tools/debug/load-and-exit.gdb $in",
            }
            ...
            buildConf:run {
                ...
                input = finalOutputs[1],
                command =
                [[ openocd -f interface/cmsis-dap.cfg -f target/rp2040.cfg -f tools/debug/openocd-gdb.config -c "adapter speed 5000" -c "program build/ginnungagap.elf reset exit"]]
            }
            ...
            buildConf:step(
                buildConf:command("svad.lua configure") {
                    input = "build.lua",
                    target = "build.ninja",
                    env = { generator = "true" }
                })

            buildConf:defaults(finalOutputs)

            buildConf:export(NINJA_FILE)
    ...
    stage2 = function(inputPath, outputPath)
        ...
        local bytes = padTo({ string.byte(contents, 1, #contents) }, 252)
        local checksum = crc32(bytes)
    ...
    uf2 = function(inputPath, outputPath)
        local inputFile = io.open(inputPath, "rb")
        ...
        local uft2Data = generateUf2({ string.byte(contents, 1, #contents) })
        local outputFile = io.open(outputPath, "wb")
